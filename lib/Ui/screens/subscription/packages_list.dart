    ...
    return BlurredRouter(
      builder: (context) {
        return BlocProvider(
          ...
          child: const SubsctiptionPackageListScreen(),
        );
      },
    );
  }
...
class _SubsctiptionPackageListScreenState
  ...
  dynamic ifServiceUnlimited(int text, {dynamic remining}) {
    if (text == 0) {
  ...
  Widget currentPackageTile(
    ...
    log("THIS IS CURRENT PACKAGE TILE");
    log("NAME:$name , ${double.parse(advertismentRemining) / advertismentLimit}");
          ...
          Padding(
            ...
            child: UiUtils.buildButton(context, onPressed: () async {
              if (mySubscriptions.isEmpty) {
                log("AppSettings.enabledPaymentGateway ${AppSettings.enabledPaymentGatway}");
                ...
                if (proceed == true) {
                  Future.delayed(
                    Duration.zero,
                    () {
                      log("AppSettings.enabledPaymentGatway ${AppSettings.enabledPaymentGatway}");
                      PaymentService paymentService = PaymentService();
                      paymentService.targetGatwayKey =
                          AppSettings.enabledPaymentGatway;
                      paymentService.attachedGatways(gatways);
